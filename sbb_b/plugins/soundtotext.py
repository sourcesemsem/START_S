...
@sbb_b.ar_cmd(pattern="اتكلم(?:\s|$)([\s\S]*)",
...
async def _(event):
    ...
    sbb_bevent = await edit_or_reply(event, "`يجري تنزيل الملف...`")
    oggfi = await event.client.download_media(reply, Config.TEMP_DIR)
    await sbb_bevent.edit("`يجري تحويل الكلام الى نص....`")
    r = sr.Recognizer()
    ...
    await sbb_bevent.edit(string_to_show)
...
@sbb_b.ar_cmd(pattern="استخرج(?:\s|$)([\s\S]*)",
...
async def _(event):
    ...
    pic_file = await sbb_b.download_media(reply, Config.TMP_DOWNLOAD_DIRECTORY)
    if not pic_file:
