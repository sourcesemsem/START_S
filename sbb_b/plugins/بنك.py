...
@sbb_b.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@sbb_b.ar_cmd(pattern="توب الفلوس(.*)")
...
@sbb_b.ar_cmd(pattern="مسح حسابي(.*)")
...
@sbb_b.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@sbb_b.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@sbb_b.ar_cmd(func=lambda m:"راتب")
...
- مثال: .انشاء حساب الرافدين او بنك كرستين
1- .استثمار (مبلغ) 
- مثال : استثمار 18276
2- .حظ (المبلغ)
- مثال : حظ 17267
...
@sbb_b.ar_cmd(pattern="انشاء حساب (.*)")
async def bankar(message):
    ...
    if input == "بنك سيمو":
    	...
    	bankn = "مصرف الرافدين"
    elif input != "الرافدين" or "بنك كرستين":
